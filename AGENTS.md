...
- All complete application blueprints belong under `blueprints/`.
- Preserve existing example paths unless a separately approved migration
  provides redirects.
- When a blueprint supersedes an example, keep the old directory as a
  compatibility entry that links to the blueprint. Do not maintain two
  divergent reference implementations.
